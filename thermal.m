...
file0 = fopen('thermal_boc_nuscale.txt','w');
PDZ1 = importdata('qboc_nuscale.txt');
Z1 =  importdata('zaxis.txt');
...
%Heat linear[W]
f=linspace(0,radius_fuel,length(Q3));
g=linspace(radius_fuel,radius_fuel+thickness_gap,length(Q3));
c=linspace(radius_fuel+thickness_gap,radius_fuel+thickness_gap+thickness_clad,length(Q3));
o=linspace(radius_fuel+thickness_gap+thickness_clad,radius_fuel+thickness_gap+thickness_clad+thickness_oxidus,length(Q3));
m=linspace(radius_fuel+thickness_gap+thickness_clad,radius_fuel+1.2*thickness_gap+1.2*thickness_clad,length(Q3));

Tmod=((max(Q2R)*f(end)^2)/(2*(thickness_gap+thickness_oxidus+thickness_clad+f(end))*(H_fluid)))+Tm_in;
To=((max(Q2R)*f(end)^2)/((f(end)+thickness_oxidus)*(K_oxidus./thickness_oxidus)))+Tmod;
Tc=((max(Q2R)*f(end)^2)/(2*K_clad))*log((f(end)+thickness_clad)/(f(end)+thickness_gap))+To;
Tg=((max(Q2R)*f(end)^2)/(2*(f(end)+thickness_gap)*H_gap))+Tc;
Tcl=((max(Q2R)*f(end)^2)/(4*K_fuel))+Tg;

for i=1:length(Q3);
T_mod(i)=Tmod-((max(Q2R)*f(i)^2)/(2*(thickness_gap+thickness_oxidus+thickness_clad+f(i))*(H_fluid)));
T_o(i)=To-((max(Q2R)*f(i)^2)/(2*K_clad))*log((f(i)+thickness_clad)/(f(end)+thickness_gap));
T_c(i)=Tc-((max(Q2R)*(f(i)^2)*log((f(i)+thickness_clad)/f(i)))/(2*K_clad));
...
plot(1e3*f,T_cl,1e3*g,T_g,1e3*c,T_c,1e3*o,T_o,1e3*m,T_mod)
legend('fuel','gap','cladding','oxidus','coolant')
...
plot(z_axis,Ta_1,z_axis,Ta_2,z_axis,Ta_3,z_axis,Ta_4)
legend('1-coolant','2-cladding','3-gap','4-fuel')
